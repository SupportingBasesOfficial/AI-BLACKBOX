...
const LOGIC_CORE_PATTERNS = [
  ...
  /builder/i, /strategy/i, /validator/i, /calculator/i,
  /service/i, /worker/i, /job/i, /task/i, /listener/i,
];
...
const INGRESS_FILENAMES = [
  "router", "routes", "controller", "handler", "resolver",
...
function collectAllFiles(scanResult) {
  ...
  for (const schema of scanResult.schemas) {
    files.push({ path: schema.file, type: schema.type, name: schema.name });
  }

  for (const route of scanResult.routes) {
    files.push({ path: route.file, type: "route", name: route.file });
  }

  for (const comp of scanResult.components) {
    files.push({ path: comp.file, type: "component", name: comp.file });
  }

  for (const model of scanResult.models) {
    files.push({ path: model.file, type: "orm_model", name: model.file });
  }
...
function classifyFile(file, scanResult) {
  ...
  for (const pattern of LOGIC_CORE_PATTERNS) {
    if (pattern.test(fileName) || pattern.test(dirName)) return "logic-core";
  }
