...
const ENV_NOISE = new Set([
  ...
  "PWD", "HOME", "PATH", "SHELL", "TERM", "USER", "LANG", "LC_ALL",
  "APPDATA", "LOCALAPPDATA", "PROGRAMFILES", "PROGRAMDATA", "TEMP", "TMP",
  "NODE_ENV", "npm_config_node_version", "npm_config_prefix",
]);
...
export function scanProject(rootDir) {
  ...
  for (const file of files) {
    ...
    const componentMatches = matchComponents(content, ext);
...
function matchComponents(content, ext) {
      ...
      for (const entry of entries) {
        ...
        if (existsSync(pkgJsonPath)) {
          ...
          for (const subDir of subDirs) {
            const subPkgPath = join(entryPath, subDir, "package.json");
            if (existsSync(subPkgPath)) {
              try {
                const pkg = JSON.parse(readFileSync(subPkgPath, "utf-8"));
                packages.push({
                  name: pkg.name || `${entry.name}/${subDir}`,
                  path: `${dir}/${entry.name}/${subDir}`,
                });
              } catch {
                packages.push({ name: `${entry.name}/${subDir}`, path: `${dir}/${entry.name}/${subDir}` });
              }
            }
          }
