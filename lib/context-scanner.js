...
const SCHEMA_PATTERNS = [
  ...
  { pattern: /drizzle\/.*\.sql$/i, type: "sql_migration", extractor: extractSqlTables },
  { pattern: /supabase\/migrations?\/.*\.sql$/i, type: "sql_migration", extractor: extractSqlTables },
  ...
  { pattern: /drizzle\/.*\.(ts|js)$/i, type: "code_migration", extractor: extractCodeMigrations },
...
// Business flow classification — maps keywords to semantic flows.
// Instead of just "keyword: admin", paths are classified by business domain.
const BUSINESS_FLOWS = [
  { flow: "authentication", keywords: ["auth", "password", "token", "session", "oauth", "login", "logout"] },
  { flow: "authorization", keywords: ["permission", "role", "rbac", "acl", "access-control"] },
  { flow: "admin", keywords: ["admin"] },
  { flow: "payment", keywords: ["payment", "billing", "checkout", "invoice", "charge", "refund", "stripe", "paypal"] },
  { flow: "user-management", keywords: ["user"] },
  { flow: "security", keywords: ["security", "encrypt", "decrypt", "vulnerability", "cve"] },
  { flow: "data-mutation", keywords: ["transaction", "create", "delete", "update", "write"] },
];

function classifyBusinessFlow(pathLower) {
  for (const { flow, keywords } of BUSINESS_FLOWS) {
    for (const kw of keywords) {
      if (pathLower.includes(kw)) return flow;
    }
  }
  return null;
}
...
export function scanProject(rootDir) {
  ...
  for (const file of files) {
    ...
    // Skip test files — they're not critical production paths
    if (isTestFile(file.relativePath)) continue;
    for (const keyword of CRITICAL_PATH_KEYWORDS) {
      if (pathLower.includes(keyword)) {
        // Paths are normalized to forward slashes — use "/" not sep
        ...
        if (!criticalPaths.some(cp => cp.path === criticalPath)) {
          // Classify by business flow semantics, not just keyword
          const flow = classifyBusinessFlow(pathLower) || keyword;
          criticalPaths.push({ path: criticalPath, keyword, flow });
        }
      }
    }
  }
