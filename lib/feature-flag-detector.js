...
const SCHEMA_FIELD_NAMES = new Set([
  ...
  "object", "array", "Date", "bigint", "json", "jsonb",
]);
...
function stripTypeBlocks(content) {
  ...
  // Strip `interface X { ... }` and `export interface X { ... }` blocks
  // (including optional extends). The (?:export\s+)? prefix ensures we
  // consume the "export " keyword too, avoiding leftover text.
  result = stripBracedBlocks(result, /\b(?:export\s+)?interface\s+\w+(?:\s+extends\s+[\w<>,\s]+)?\s*\{/g);

  // Strip `type X = { ... }` and `export type X = { ... }` blocks (including generics)
  result = stripBracedBlocks(result, /\b(?:export\s+)?type\s+\w+\s*(?:<[^>]*>)?\s*=\s*\{/g);
