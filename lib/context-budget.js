...
const BUDGET_LIMIT_BYTES = 8192;
const BUDGET_WARN_BYTES = 6144;
...
export function buildRulesFile(zeroErrorDir, gitDiffModule = null) {
  ...
  if (totalBytes > BUDGET_LIMIT_BYTES) {
    ...
    total = total.substring(0, cutPoint > 0 ? cutPoint : BUDGET_LIMIT_BYTES - 100) +
      "\n\n... (hard-truncated to fit 8KB budget. Run `node .zero-error/init.js --force` to regenerate.)";
    totalBytes = Buffer.byteLength(total, "utf-8");
  }
