...
const LOGIC_CORE_FILENAME_TOKENS = new Map([
  ...
  // "connector" is context-dependent: in lib/ it's business logic (integration
  // with field mapping, transformations), in data/db/ it's data-access.
  // Handled via directory-aware logic in classifyPath, not as a flat token.
  // Pattern
...
// --- Infrastructure package signals ---
// Monorepo packages like packages/logger, packages/telemetry, packages/secrets
// are infrastructure. Their barrel files (index.ts) should be classified as
// state-store (infrastructure layer) rather than left unclassified.
const INFRA_PKG_SIGNALS = [
  { re: /(?:^|\/)packages\/logger\//i,     weight: 3 },
  { re: /(?:^|\/)packages\/telemetry\//i,  weight: 3 },
  { re: /(?:^|\/)packages\/secrets\//i,    weight: 3 },
  { re: /(?:^|\/)packages\/config\//i,     weight: 3 },
  { re: /(?:^|\/)packages\/db\//i,         weight: 7 },
  { re: /(?:^|\/)packages\/cache\//i,      weight: 7 },
  { re: /(?:^|\/)packages\/queue\//i,      weight: 7 },
  { re: /(?:^|\/)packages\/storage\//i,    weight: 7 },
  { re: /(?:^|\/)packages\/redis\//i,      weight: 7 },
];
...
export function classifyPath(relativePath, fileType) {
  ...
  // --- Barrel/entry file handling ---
  // index.ts/index.js files are usually package barrel files that re-export
  // other modules. They are NOT route handlers even if the content scanner
  // detected route patterns (those patterns come from re-exported route
  // modules, not from the index file itself defining routes).
  //
  // EXCEPTION: The main API entry point (apps/api/src/index.ts or similar)
  // IS a real route handler — it creates the Hono/Express app and mounts
  // routes. We distinguish by: if the file is in apps/*/src/ AND the scanner
  // detected route content, it's a real entry point, not a barrel.
  const isBarrelFile = fullName === "index.ts" || fullName === "index.js" ||
                       fullName === "index.tsx" || fullName === "index.jsx";
  const isAppEntryPoint = isBarrelFile && fileType === "route" &&
    /(?:^|\/)apps\/[^/]+\/src\//i.test(pathLower);
  ...
  if (fileType === "route" || fileType === "openapi_path") {
    if (isBarrelFile && !isAppEntryPoint) {
      // Package barrel files re-export, they don't define routes.
      // Only app entry points (apps/*/src/index.ts with route content) pass.
    ...
    } else if (/(?:^|\/)middleware\//i.test(pathLower) || fullName === "middleware.ts" || fullName === "middleware.js") {
      // File is in middleware/ or is a Next.js root middleware.ts — it's
      // middleware, not a route, even if the scanner detected route patterns.
      addSignal(scores, "ingress", "middleware", 5, "scanner-type:route-overridden-by-middleware-name");
    } else {
      addSignal(scores, "ingress", "route", 5, "scanner-type:route");
    }
  }
  if (fileType === "component") {
    // Barrel files (index.tsx) in packages/ are package entry points, not
    // components. They re-export components. Skip the component signal.
    if (isBarrelFile && /(?:^|\/)packages\//i.test(pathLower)) {
      // Package barrel — don't classify as component
    } else {
  ...
  // Infrastructure package signals — classify infra packages as state-store
  for (const sig of INFRA_PKG_SIGNALS) {
    if (sig.re.test(pathLower)) {
      addSignal(scores, "state-store", "infrastructure", sig.weight, `infra-pkg:${sig.re.source}`);
    }
  }
  ...
  // React hook filename pattern — files starting with "use-" are hooks
  // (use-push-notifications.ts, use-auth.ts). These are frontend utilities,
  // not business logic. They should not get logic-core signals.
  const isReactHook = /^use[-_]/i.test(fullName);
  ...
  for (const token of tokens) {
    ...
    if (LOGIC_CORE_FILENAME_TOKENS.has(token)) {
      // Skip logic-core tokens for config files and React hooks
  ...
  // Penalize logic-core for React hooks
  if (isReactHook) {
    for (const key of Object.keys(scores)) {
      if (key.startsWith("logic-core:")) {
        scores[key].weight -= 3;
        scores[key].signals.push({ source: "react-hook-penalty", weight: -3 });
      }
    }
  }

  // --- Directory-aware "connector" token ---
  // "connector" is ambiguous: in lib/ it's an integration connector (ITSM,
  // Jira, ServiceNow) with field mapping and transformation logic; in data/db/
  // it's data-access (database connector). We use "integration" subtype for
  // the former to distinguish from pure business logic.
  if (tokens.includes("connector")) {
    if (/(?:^|\/)(lib|libs|services|domain|business|integrations?)\//i.test(pathLower)) {
      // connector in lib/services/domain = integration logic (external system
      // integration with field mapping, transformations, protocol handling)
      // Weight 4 to overcome dir signals (services/ = 3) that would classify
      // as generic "business" — "connector" is more specific than "services".
      addSignal(scores, "logic-core", "integration", 6, "dir-aware:connector-in-lib");
    } else if (/(?:^|\/)(data|db|database|packages\/db|infrastructure)\//i.test(pathLower)) {
      // connector in data/db = data access
      addSignal(scores, "logic-core", "data-access", 6, "dir-aware:connector-in-data");
    } else {
      // connector in other dirs (e.g. root, src/) = default to integration
      addSignal(scores, "logic-core", "integration", 2, "dir-aware:connector-default");
    }
  }
