...
const INFRA_PKG_SIGNALS = [
  ...
  { re: /(?:^|\/)packages\/db\//i,         weight: 4 },
  { re: /(?:^|\/)packages\/cache\//i,      weight: 4 },
  { re: /(?:^|\/)packages\/queue\//i,      weight: 4 },
  { re: /(?:^|\/)packages\/storage\//i,    weight: 4 },
  { re: /(?:^|\/)packages\/redis\//i,      weight: 4 },
];
...
export function classifyPath(relativePath, fileType) {
  ...
  // "connector" is ambiguous: in lib/ it's business logic (integration with
  // field mapping, transformations — e.g. itsm-connector.ts maps ITSM fields);
  // in data/db/packages/db/ it's data-access (e.g. db-connector.ts connects
  // to a database). We resolve this by checking the directory context.
  if (tokens.includes("connector")) {
    if (/(?:^|\/)(lib|libs|services|domain|business|integrations?)\//i.test(pathLower)) {
      // connector in lib/services/domain = business integration logic
      addSignal(scores, "logic-core", "business", 2, "dir-aware:connector-in-lib");
    } else if (/(?:^|\/)(data|db|database|packages\/db|infrastructure)\//i.test(pathLower)) {
      // connector in data/db = data access
      addSignal(scores, "logic-core", "data-access", 2, "dir-aware:connector-in-data");
    } else {
      // connector in other dirs (e.g. root, src/) = default to business
      addSignal(scores, "logic-core", "business", 1, "dir-aware:connector-default");
    }
  }
