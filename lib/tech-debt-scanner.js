...
function detectUnusedDeps(allDeps, importRegistry) {
  ...
  const skipPrefixes = ["@types/", "eslint-", "prettier", "@babel/", "ts-node", "tsx"];

  for (const depName of Object.keys(allDeps)) {
    if (importedPkgs.has(depName)) continue;

    const shouldSkip = skipPrefixes.some(p => depName.startsWith(p) || depName === p);
...
function checkPkgHasTypes(pkgName, allDeps) {
  ...
  if (knownTypedPackages.has(pkgName)) return true;
  if (pkgName.startsWith("@fastify/") || pkgName.startsWith("@nestjs/")) return true;
  if (pkgName.startsWith("@supabase/")) return true;
  return false;
}
