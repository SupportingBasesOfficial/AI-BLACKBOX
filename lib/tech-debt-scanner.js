...
function detectUnusedExports(allFiles, rootDir) {
  ...
  for (const file of allFiles) {
    ...
    const imports = extractImports(content, ext);
    for (const imp of imports) {
      // For relative imports, extract the base name
      if (imp.startsWith(".")) {
        const base = imp.split("/").pop().replace(/\.(ts|js|tsx|jsx)$/, "");
        if (base && base !== "index") allImports.add(base);
      }
      // For @repo/* imports, extract the subpath
      if (imp.startsWith("@repo/") || imp.startsWith("@app/")) {
        const parts = imp.split("/");
        if (parts.length > 2) {
          const subPath = parts.slice(2).join("/").replace(/\.(ts|js|tsx|jsx)$/, "");
          if (subPath) allImports.add(subPath);
        }
      }
    }
  }
  ...
  for (const file of allFiles) {
    ...
    // Skip test files, config files, entry points
    if (isTestFile(file.relativePath)) continue;
    if (base.endsWith(".config") || base.endsWith(".d")) continue;
